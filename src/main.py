...
from robot_learning_agent import RobotLearningAgent
...
def should_change_layout(robots, step_counter, last_layout_change):
    ...
    min_steps_between_changes = 1000
    if step_counter - last_layout_change < min_steps_between_changes:
        return False
...
def run_improved_simulation(args, robots, env, space, evaluator):
    """Run simulation with improved task management and layout changes"""
    # Initialize pygame with improved settings
    pygame.init()
    screen = pygame.display.set_mode((800, 600))
    pygame.display.set_caption("Improved Warehouse Robot Simulation")
    ...
    # Set up colors for different states
    colors = {
        "idle": (255, 0, 0),      # Red
    ...
    while running:
        ...
        if step_counter % task_assignment_interval == 0 or step_counter == 0:
            assign_tasks(robots, env, evaluator)
        ...
        for robot in robots:
            ...
            if step_counter % 120 == robot.id * 20:  # Stagger checks across robots
        ...
        for robot in robots:
            ...
            color = colors.get(robot.state, (128, 128, 128))
        ...
        info_text = font.render(
            f"Step: {step_counter} | Robots: {args.num_robots} | Layout v{env.layout_version}", 
            True, (0, 0, 0)
        )
...
def main():
    ...
    env = WarehouseEnvironment(800, 600)
    ...
    if args.mode == 'train':
        ...
        # Train robots with improved training process
        ...
        run_improved_simulation(args, robots, env, space, evaluator)
