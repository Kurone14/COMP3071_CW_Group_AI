...
class PathStrategySelector:
    ...
    def __init__(self, grid: Grid, obstacle_manager=None):
        ...
        # Performance tracking for each strategy - **INITIAL SPEED SET TO 0.0**
        self.strategy_performance = {
            'astar': {'success_rate': 0.5, 'speed': 0.0, 'path_quality': 0.5, 'used_count': 0},
            'ad_star': {'success_rate': 0.5, 'speed': 0.0, 'path_quality': 0.5, 'used_count': 0},
            'pp_dijkstra': {'success_rate': 0.5, 'speed': 0.0, 'path_quality': 0.5, 'used_count': 0}
        }
    ...
    def _update_performance_metrics(self,
        ...
        if self.execution_times[strategy_name]:
            ...
            if all_times:
                ...
                speed = min(1.0, avg_all_time / (avg_time + 1e-6)) # Added small epsilon
            else:
                speed = 0.5  # Default
        else:
            speed = 0.5  # Default
        ...
        # Use moving average to smooth updates
        alpha = 0.1 # Learning rate
        self.strategy_performance[strategy_name]['success_rate'] = (1 - alpha) * self.strategy_performance[strategy_name]['success_rate'] + alpha * success_rate
        self.strategy_performance[strategy_name]['speed'] = (1 - alpha) * self.strategy_performance[strategy_name]['speed'] + alpha * speed
        self.strategy_performance[strategy_name]['path_quality'] = (1 - alpha) * self.strategy_performance[strategy_name]['path_quality'] + alpha * path_quality
