...
class WarehouseSimulation:
    ...
    def _initialize_components(self):
        ...
        self.obstacle_manager = ObstacleManager(self.grid, self.width, self.height)
        
        self.movement_controller = MovementController(
            self.grid, self.width, self.height, self.drop_point, self.path_finder, self.obstacle_manager
        )
        self.stall_detector = StallDetector(
            self.grid, self.width, self.height, self.drop_point, self.path_finder
        )
        
        # Update path_finder to use obstacle manager
        if self.path_finder:
            self.path_finder.obstacle_manager = self.obstacle_manager
    ...
    def initialize_environment(self, robot_count, item_count):
        ...
        # Initialize obstacle manager with the grid
        if self.obstacle_manager:
            self.obstacle_manager.update_grid(self.grid)
        ...
        for y in range(self.height):
            for x in range(self.width):
                if random.random() < self.obstacle_density:
                    self.grid[y][x] = 1  # Permanent obstacle
    ...
    def _setup_drop_point(self):
        ...
        self.grid[drop_y][drop_x] = 4  # Drop point
    ...
    def toggle_obstacle(self, x, y):
        ...
        if self.grid[y][x] not in [0, 1, 5, 6]:  # Don't toggle entities
            print(f"Cannot toggle obstacle at ({x}, {y}): position is occupied by entity")
            return False
        
        if self.obstacle_manager:
            if self.grid[y][x] == 0:
                # Add permanent obstacle
                result = self.obstacle_manager.add_obstacle(x, y, obstacle_type=1)
                if result:
                    print(f"Added permanent obstacle at ({x}, {y})")
            else:
                # Remove any type of obstacle
                result = self.obstacle_manager.remove_obstacle(x, y)
                if result:
                    print(f"Removed obstacle at ({x}, {y})")
        else:
            # Legacy behavior without obstacle manager
            if self.grid[y][x] == 0:
                self.grid[y][x] = 1  # Set to obstacle
    ...
    def add_temporary_obstacle(self, x, y, lifespan=10):
        ...
        if self.grid[y][x] not in [0, 1, 5, 6]:  # Don't overwrite entities
            print(f"Cannot add obstacle at ({x}, {y}): position occupied by entity")
            return False
        
        result = self.obstacle_manager.add_temporary_obstacle(x, y, lifespan)
    ...
    def add_semi_permanent_obstacle(self, x, y, lifespan=30):
        ...
        if not (0 <= x < self.width and 0 <= y < self.height):
            print(f"Invalid obstacle coordinates: ({x}, {y})")
            return False
        ...
        result = self.obstacle_manager.add_semi_permanent_obstacle(x, y, lifespan)
    ...
    def resize_grid(self, new_width, new_height):
        ...
        if self.obstacle_manager:
            self.obstacle_manager.grid = new_grid
            self.obstacle_manager.width = new_width
            self.obstacle_manager.height = new_height
    ...
    def reset_simulation(self):
        ...
        for y in range(self.height):
            for x in range(self.width):
                if self.grid[y][x] in [1, 4, 5, 6]:  # Preserve obstacles and drop point
                    preserved_grid[y][x] = self.grid[y][x]
        ...
        if self.obstacle_manager:
            self.obstacle_manager.update_grid(preserved_grid)
        ...
        for robot in self.entity_manager.robots:
            ...
            if 0 <= robot.y < self.height and 0 <= robot.x < self.width:
                self.grid[robot.y][robot.x] = 3  # Robot
        ...
        for item in self.entity_manager.items:
            ...
            if 0 <= item.y < self.height and 0 <= item.x < self.width:
                self.grid[item.y][item.x] = 2  # Item
    ...
    def simulation_loop(self):
        ...
        # Update obstacles lifecycle
        if self.obstacle_manager:
            removed_count = self.obstacle_manager.update_cycle()
            if removed_count > 0 and self.gui:
                self.gui.draw_environment(
                    self.grid, self.width, self.height, 
                    self.drop_point, self.entity_manager.robots, self.entity_manager.items,
                    obstacle_manager=self.obstacle_manager
            ...
            if self.entity_manager.robots and len(self.entity_manager.robots) > 1:
                # Share knowledge every ~10 cycles randomly
                if random.random() < 0.1:
                    # Pick a random pair of robots to share knowledge
                    if len(self.entity_manager.robots) >= 2:
                        robots = random.sample(self.entity_manager.robots, 2)
                        self.obstacle_manager.share_obstacle_knowledge(robots[0].id, robots[1].id)
